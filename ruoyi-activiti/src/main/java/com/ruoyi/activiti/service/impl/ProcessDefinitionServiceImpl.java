...
public class ProcessDefinitionServiceImpl implements IProcessDefinitionService {
    ...
    @Override
    public Page<ProcessDefinitionDTO> selectProcessDefinitionList(ProcessDefinitionDTO processDefinitionDto, PageDomain pageDomain) {
    ...
    @Override
    public Deployment addDeploymentByString(ProcessDefinitionDTO processDefinition) {
        return repositoryService.createDeployment()
                .addString("CreateWithBPMNJS.bpmn", processDefinition.getBpmn())
                .addString("CreateWithBPMNJS.png", processDefinition.getBpmn())
                .deploy();
    }

    @Override
    public void updateDeploymentByString(ProcessDefinitionDTO processDefinitionDto) {
        // 部署修改后的流程
        Deployment deployment = addDeploymentByString(processDefinitionDto);
        ProcessDefinitionQuery processDefinitionQuery = repositoryService.createProcessDefinitionQuery();
        // 查询原流程定义信息
        ProcessDefinition processDefinition = processDefinitionQuery.processDefinitionId(processDefinitionDto.getId()).singleResult();
        ActReProcdefVO actReProcdefVO = new ActReProcdefVO();
        actReProcdefVO.setId(processDefinition.getDeploymentId());
        actReProcdefVO.setName(processDefinitionDto.getName());
        actReDeploymentMapper.updateActReProcdef(actReProcdefVO);
        // 删除新流程的部署记录
        actReDeploymentMapper.deleteActReProcdef(deployment.getId());
        // 删除原流程图信息
        actReDeploymentMapper.deleteActGeBytearray(processDefinition.getDeploymentId());
        // 修改新部署的流程图文件的部署id为原流程的部署id
        actReDeploymentMapper.updateActGeBytearray(processDefinition.getDeploymentId(),deployment.getId());
        actReDeploymentMapper.deleteActReDeployment(deployment.getId());
    }
    ...
    @Override
    public ActReProcdefVO selectActReProcdefByKey(String key) {
        return actReDeploymentMapper.selectActReProcdefByKey(key);
    }
}
